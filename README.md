...
[![GitHub stars](https://img.shields.io/github/stars/gowtham012/pinpoint)](https://github.com/gowtham012/pinpoint/stargazers)

**You point at the UI. Your coding agent gets the fix context.**

Click an element on your local dev site, write what should change, and your coding agent gets the selector, DOM path, computed styles, React/Vue component chain, source-file hint, and a cropped screenshot — no Downloads clutter, no "third button on the left."
...
## Why not a screenshot / vs Playwright MCP & agent-browser

Pinpoint is **human annotation → agent edit context**. It is not a browser driver.

- **Screenshots alone** leave the agent guessing which control you meant and where it lives in the tree.
- **Playwright MCP / agent-browser** automate or drive the browser (navigate, click, assert). Useful for testing and agent-controlled browsing — different job.
- **Pinpoint** stays out of the driver's seat: *you* click (or drag a region), type the change, and the agent gets structured context (selector, styles, component chain, source hint, crop) over a **local-only** bridge (`127.0.0.1`).
- Injection defaults to local dev hosts (`localhost`, `127.0.0.1`, `.local` / `.test`, `file://` with file access). Other tabs need an explicit popup opt-in.

## Install in minutes

**macOS (fastest):** from a clone of this repo:

```bash
bash setup.sh ~/code/my-app
```

That installs bridge deps, registers Claude Code MCP + hooks when `claude` is on PATH, opens `chrome://extensions` so you can **Load unpacked → `extension/`**, and starts the bridge. Replace `~/code/my-app` with the project whose UI you are annotating.
...
node cli.js --project ~/code/my-app          # leave running
# Chrome → chrome://extensions → Developer mode → Load unpacked → extension/
```

Then wire your agent (once). Claude Code: `claude mcp add pinpoint -s user -- node "$PWD/cli.js" mcp` plus `install-hooks` / `install-native-host` as in [Quick start](#quick-start). **Cursor / Codex:** see [Connecting other agents](#connecting-other-agents) (absolute path to `cli.js`). Need Node 18+ and a Chromium browser. Deeper steps, Windows notes, and `file://` access live in Quick start below.
...
Using **Cursor** or **Codex** instead of Claude Code? Skip the `claude mcp add` / hooks lines and wire MCP once — see [Connecting other agents](#connecting-other-agents).
